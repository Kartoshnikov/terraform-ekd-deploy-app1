...
data "terraform_remote_state" "eks" {
  backend = "http"
  config = {
    address  = "https://git.example.com/api/v4/projects/148/terraform/state/eks"
...
data "terraform_remote_state" "manager" {
  backend = "http"
  config = {
    address  = "https://git.example.com/api/v4/projects/158/terraform/state/manager"
    username = "gitlab-ci-token"
    password = var.REMOTE_STATE_PASSWORD
  }
}
...
resource "kubernetes_ingress" "app" {
  metadata {
    ...
    annotations = {
      ...
      "nginx.ingress.kubernetes.io/canary-weight"          = var.CANARY_PERCENTAGE
...
resource "kubernetes_secret" "app" {
  metadata {
    name      = "gitlab-pull-secret-canary"
    namespace = var.NAMESPACE
  }
  data = {
